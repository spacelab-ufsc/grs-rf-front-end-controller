...
def interpret_command():
    ...
    and toggles the pins associated with the required hardware.
    """
    rcv_json = request.get_json()
    
    if not rcv_json or 'status' not in rcv_json:
        return 1
    ...
    # Telemetry
    if rcv_json.get('request_telemetry'):
        response_data["telemetry"] = read_sensors()
